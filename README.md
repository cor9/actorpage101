...
Actor sites are published at `{slug}.actorpage101.site`. The middleware rewrites these requests to `/site/[slug]`, which:
- Fetches site data from Supabase
- Uses ISR (Incremental Static Regeneration) for performance
- Renders with the selected template and color preset
